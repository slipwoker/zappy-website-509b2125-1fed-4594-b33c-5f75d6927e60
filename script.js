...
// Booking steps navigation (if on booking page)
...
if (bookingSteps.length > 0) {
  ...
  document.querySelectorAll('.participant-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      document.querySelectorAll('.participant-btn').forEach(b => b.classList.remove('selected'));
      this.classList.add('selected');
...
// Availability tabs
document.querySelectorAll('.availability-pill').forEach(pill => {
  pill.addEventListener('click', function() {
    document.querySelectorAll('.availability-pill').forEach(p => p.classList.remove('active'));
...
document.querySelectorAll('.clue-stone, .stat-stone, .gallery-tile, .testimonial-chip, .quote-chip').forEach(el => {
...
document.querySelectorAll('.contact-form').forEach(form => {
  form.addEventListener('submit', function(e) {
    ...
    successMsg.style.cssText = 'background: var(--accent); color: var(--text-dark); padding: 1rem; border-radius: var(--radius-md); margin-top: 1rem; text-align: center; font-weight: 700;';
    successMsg.textContent = 'תודה! נחזור אליכם בהקדם.';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Ring of Fire משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
        ...
        function initContactForm() {
            ...
            try {
                ...
                const response = await fetch(endpoint, {
                    ...
                    body: JSON.stringify({
                        websiteId: '509b2125-1fed-4594-b33c-5f75d6927e60',
                        name: data.name,
                        email: data.email,
